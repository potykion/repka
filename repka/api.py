...
class SQLiteRepo(BaseRepository, Generic[T], ABC):
    ...
    async def exists(self, *filters: BinaryExpression) -> bool:
        query = sa.select([sa.func.count("*")])
        query = self._apply_filters(query, filters)
        result = await self.connection.fetch_val(query)
        return bool(result)
    ...
    async def insert(self, entity: T) -> T:
        ...
        query = self.table.insert().values(serialized)
        id_ = await self.connection.execute(query)

        entity.id = id_

        # if self.ignore_insert:
        #     id__ = await self.get_by_id(id_)
        #     entity = id__
        #     assert entity
