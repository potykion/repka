...
class TransactionRepoWithConnectionMixin(DatabasesRepository[Transaction]):
    table = transactions_table
...
class UnionModelRepo(DatabasesRepository[UnionModel]):
...
async def test_connection_var_mixin_allows_to_create_repo_without_connection_if_connection_var_is_third_party(
    ...
    new_db_connection_var: ContextVar[Database] = ContextVar("new_db_connection_var")
...
async def test_insert_many_in_transaction_rollback_on_error(conn: Database) -> None:
...
async def test_error_in_transaction_inside_transaction_rollback(conn: Database) -> None:
    repo = UnionModelRepo(conn)
